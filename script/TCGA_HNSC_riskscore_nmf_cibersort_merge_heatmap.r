...
######################
## you could run CIBERSORT at https://cibersort.stanford.edu/runcibersort.php (Permutations = 1000) using "hnsc_tpm_for_cibersort.txt" as input to get CIBERSORT output file 
fiber_res<-read.table("OUT/TCGA_HNSC_CIBERSORT_result.txt",
                      sep = "\t",header = T,stringsAsFactors = F,check.names = F)

rownames(fiber_res)<-fiber_res$`Input Sample`
HNSC_ciber <- fiber_res[,2:23]
...
gene_score_15<-read.table("OUT/hnsc_tpm_15gene_signature_scores_groupsBy3cutoffs.txt",
                          sep = "\t",header = T,stringsAsFactors = F,check.names = F)
##
...
saveRDS(rcc6, file = "OUT/TCGA_HNSC_CIBERSORT_ConsensClusterPlus_maxK_6.rds")
rcc6<-readRDS(file="OUT/TCGA_HNSC_CIBERSORT_ConsensClusterPlus_maxK_6.rds")
...
pdf("OUT/HNSC_cibersort_consensusClass_2clusters_survival.pdf",width=6.5,height=5.5)
fit_immune_os<-survfit(Surv(OS_time,OS_stage)~ICI,data = ann3)
ggsurvplot(fit_immune_os, data = ann3, pval = T,legend.labs = levels(ann3$ICI),
           legend = "top",surv.median.line="hv",risk.table = T,palette = c("#2E9FDF","#E7B800"),
           legend.title = "",
           title = "Immune_subtype (n=519)",
...
#########
pdf(file="OUT/HNSC_cibersort_consensusClass_boxplot.pdf",width=6,height=3)
ggarrange(p1, p2, p3, 
          ncol = 3, nrow = 1)
dev.off()
...
pdf(file="OUT/HNSC_cibersort_consensusClass_cells_boxplot.pdf",width=8,height=5.5)
...
multi_var<-read.table("OUT/tcga_hnsc_signature_scores_groupsBy3cutoffs_multivariate_cli.txt",
                      sep = "\t",header = T,stringsAsFactors = F,check.names = F)

multi_var<-merge(ann3,multi_var,by.x="ann_samp",by.y="SampleID",all.x=T)
multi_var$HPV[is.na(multi_var$HPV)]<-"N/A"
multi_var$HPV<-factor(multi_var$HPV,levels = c("HPV16","HPV33","Other HPVs","Negative","N/A"))
multi_var$Primary_site<-factor(multi_var$Primary_site,levels = c("OPSCC","Non-OPSCC"))
multi_var$Pathologic_stage[is.na(multi_var$Pathologic_stage)]<-"N/A"
multi_var$Pathologic_stage<-factor(multi_var$Pathologic_stage,levels = c("I-II","III","IV","N/A"))
multi_var$Smoking[is.na(multi_var$Smoking)]<-"N/A"
multi_var$Smoking<-factor(multi_var$Smoking,levels = c("No","Yes","N/A"))
multi_var$Margin_status[is.na(multi_var$Margin_status)]<-"N/A"
multi_var$Margin_status<-factor(multi_var$Margin_status,levels = c("Negative","Close","Positive","N/A"))
multi_var$Radiation_therapy[is.na(multi_var$Radiation_therapy)]<-"N/A"
multi_var$Radiation_therapy<-factor(multi_var$Radiation_therapy,levels = c("No","Yes","N/A"))
multi_var$Histological_grade[is.na(multi_var$Histological_grade)]<-"N/A"

table(multi_var$Histological_grade)

multi_var$Histological_grade<-factor(multi_var$Histological_grade,levels = c("G1","G2",'G3-G4',"GX","N/A"))

write.table(multi_var,"OUT/tcga_hnsc_signature_scores_groupsBy3cutoffs_multivariate_cli_NMFs_ICIs.txt",sep = "\t",quote = F)

pdf("OUT/HNSC_cibersort_Heatmap_split_by_riskscore.pdf",width=12,height=7)
Heatmap(scaled_mat,column_km_repeats = 500,name = " ",
        top_annotation = HeatmapAnnotation(
          simple_anno_size = unit(0.35, "cm"),
          HPV=multi_var$HPV,Primary_site=multi_var$Primary_site,Pathologic_stage=multi_var$Pathologic_stage,
          Smoking=multi_var$Smoking,Margin_status=multi_var$Margin_status,Radiation_therapy=multi_var$Radiation_therapy,
          Histological_grade=multi_var$Histological_grade,StromalScore=multi_var$StromalScore,ImmuneScore=multi_var$ImmuneScore,
          ESTIMATEScore=multi_var$ESTIMATEScore,NMF_cluster=multi_var$NMF_cluster,Risk_score=multi_var$group,
          ICI=multi_var$ICI,
          col=list(
            HPV=c("HPV16"="coral3","HPV33"="green3","Other HPVs"="lightpink","Negative"="cadetblue1","N/A"="grey"),
            Primary_site=c("OPSCC"="coral3","Non-OPSCC"="yellowgreen"),
            Pathologic_stage=c("I-II"="cadetblue1","III"="plum1","IV"="brown1","N/A"="grey"),
            Smoking=c("No"="cadetblue1","Yes"="#E7B800","N/A"="grey"),
            Margin_status=c("Negative"="cadetblue1","Close"="plum1","Positive"="brown1","N/A"="grey"),
            Radiation_therapy=c("No"="cadetblue1","Yes"="#E7B800","N/A"="grey"),
            Histological_grade=c("G1"="cadetblue1","G2"="plum1",'G3-G4'="brown1","GX"="grey1","N/A"="grey"),
...
pdf("OUT/HNSC_signature_ici_nmf_survival_mix.pdf",width=5.5,height=3)
